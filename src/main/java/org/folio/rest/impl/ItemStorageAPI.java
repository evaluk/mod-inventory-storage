    ...
    try {
      vertxContext.runOnContext(v -> {
        try {
          ...
          CQLWrapper cql = getCQL(query,limit+1,offset);
          String filter = null;
          ////// temporary hack to fix jsonb syntax generated by cql
          ///// to improve sort performance by an order of magnitude
          if(cql != null){
            int []sortClause = new int[]{1};
            String where[] = cql.toString().split(" ");
            StringBuffer sb = new StringBuffer();
            for (int i = 0; i < where.length; i++) {
              if(where[i].contains("ORDER")){
                sortClause[0] = 1;
              }
              else if(where[i].contains("BY") && sortClause[0] == 1){
                sortClause[0] = sortClause[0]+1;
              }
              else if (sortClause[0] != 2){
                sortClause[0] = 0;
              }
              if(where[i].contains("->'") && sortClause[0] == 2){
                int start = where[i].lastIndexOf("->'");
                String rep = where[i].substring(0, start) + where[i].substring(start).replace("->", "->>");
          ...
          postgresClient.get(getTableName(query), Item.class, fieldList, filter, false, false,
            reply -> {
              try {
                ...
                if(reply.succeeded()) {
                  ...
                  if(items.size() == limit+1){
                    //there are more results that can be queried
                    //remove the +1 record that was not requested but was used to
                    //verify that there are more results
                    items.remove(limit);
                    itemList.setTotalRecords(offset+(2*limit));
                  }
                  else{
                    //set total results if they are less then the limit
                    //we've reached the total amount of available records for this query
                    //as the offset + limit + 1 has not been reached
                    itemList.setTotalRecords(offset+items.size());
                  }
